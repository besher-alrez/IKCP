    ...
    <b-card>
      ...
      >
        ...
        >
          ...
          <template v-slot:cell(HEAD_employee_name)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(HEAD_attendance_date)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(HEAD_dept_movement)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(HEAD_hr_review)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(HEAD_ey_review)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(HEAD_remarks)="data">
            <p>{{ data.label }}</p>
          </template>
          <template v-slot:cell(index)="data">
            {{ data.index + 1 }}
          </template>
          <template v-slot:cell(dept_movement)="row">
            <b-badge :variant="status_color_map[row.value]">
              {{ $t(`labels.backend.attendances.statuses.${row.value}`) }}
            </b-badge>
          </template>
          <template v-slot:cell(logged_in_time)="row">
          ...
          <template v-slot:cell(logged_out_time)="row">
            <span v-if="!row.item.editable">{{ row.value }}</span>
            <p-datetimepicker
              :config="time_config"
              id="flight_time"
          ...
          <template v-slot:cell(hr_review)="row">
            <span v-if="!row.item.editable">{{ row.value }}</span>
            <b-form-radio-group
              v-else
              id="hr_review"
          ...
          <template v-slot:cell(ey_review)="row">
            <span v-if="!row.item.editable">{{ row.value }}</span>
            <b-form-input
              v-else
...
export default {
  ...
  mounted() {
    ...
    let headers = this.$refs.datatable.$el.querySelectorAll('thead tr')
    let top = this.$refs.datatable.$el.querySelectorAll('tbody tr')
    console.log(headers, top, headers[0].parentNode, top[0])
    if (headers.length > 1) {
